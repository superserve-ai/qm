...
| `SANDBOX_BACKEND`                                                                                                         | yes      | Mandatory in production. This fork's core accepts `aws`, `local`, `sprites`, `smolmachines`, `e2b`, `modal`, `porter`, `agent37`. See the note below on Superserve.                       |
| `SUPERSERVE_API_KEY`, `SUPERSERVE_BASE_URL`, `SUPERSERVE_TEMPLATE`                                                        | reserved | Reserved for the Superserve sandbox backend. Core does not read them yet; the supervisor passes them through unchanged.                                                                   |
...
Superserve sandbox backend: `SANDBOX_BACKEND=superserve` is not recognised by `src/config.ts` in this fork (`sandboxBackendEnvStrict` rejects it at boot). Until that backend lands, hosted tenants need one of the accepted values plus its credential (`SPRITES_TOKEN`, `E2B_API_KEY`, `MODAL_TOKEN_ID`+`MODAL_TOKEN_SECRET`, `PORTER_DEPLOY_API_TOKEN`+locator, `AGENT37_API_KEY`, `SMOLMACHINES_TOKEN`, or `AWS_SANDBOX_S3_BUCKET` for `aws`). `local` is the only backend that constructs without credentials; it shells out to a Docker daemon at first use, which this image does not have, so it is fit for the local harness only.
